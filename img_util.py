...
def zero_pad(cnn_model, dimension):
    last_layer = cnn_model.get_layer(index=-1)
    layer_config = last_layer.get_config()
    
    if 'pool_size' not in layer_config.keys():
        raise TypeError('zero_pad function should be used after a pooling layer.')
    
    if dimension == 2:
        padding_func = ZeroPadding2D
        pool_size = layer_config.get('pool_size')[0]
    elif dimension == 3:
        padding_func = ZeroPadding3D
        pool_size = layer_config.get('pool_size')
    else:
        raise ValueError('Unsupported dimension. Supported dimensions are 2 and 3.')
    
    if pool_size != layer_config.get('strides'):
        raise ValueError('Strides must be equal to pool_size in the pooling layer.')
   
    output_shape = last_layer.get_output_shape_at(0)
    remainder = output_shape[-2] % pool_size[-1] if dimension == 3 else output_shape[-1] % pool_size
    if remainder:
        padding_dim = pool_size - remainder
        if dimension == 3:
            padding = ((0,0), (0,0), (0,padding_dim))
        else:
            padding = ((0,0), (0,0), (0,0), (0,padding_dim))
        cnn_model.add(layer=padding_func(padding=padding))
